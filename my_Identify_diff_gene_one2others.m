function [DG]=my_Identify_diff_gene_one2others(M_scaled,S,gene,meth_type,isplot)
if nargin<3
...
switch meth_type
    case 1
        ...
        for i=1:cluster_num            
                ...
                mean_j=mean(M_scaled(:,find(S~=i)),2);
                FC=log2(mean_i./mean_j);% FC
                u=find(S==i);
                if length(u)==1
                    u=[u u]; % in case that only one cell in the cluster.
                end
                tLocal = nbintest(M_scaled(:,u),M_scaled(:,find(S~=i)),'VarianceLink','LocalRegression');
                p=tLocal.pValue;
                nlg_p=-log10(p);
                [FDR] = mafdr(p,'Showplot', false);% FDR estimation
                % write into files
                file_in=strcat('NB Model for-',num2str(i),'-VS-others','.txt');
                disp(strcat('NB Model for-',num2str(i),'-VS-others'))
    ...
    case 2 %'Identity'
        ...
        for i=1:cluster_num            
                ...
                mean_j=mean(M_scaled(:,find(S~=i)),2);
                FC=log2(mean_i./mean_j);% FC
                tLocal = nbintest(M_scaled(:,find(S==i)),M_scaled(:,find(S~=i)),'VarianceLink','Identity');
                p=tLocal.pValue;
                nlg_p=-log10(p);
                [FDR] = mafdr(p,'Showplot', false);% FDR estimation
                % write into files
                file_in=strcat('Possion Model Model for-',num2str(i),'-VS-others','.txt'); 
                disp(strcat('Possion Model Model for-',num2str(i),'-VS-others'))
                write2file_NB_P(gene,mean_i,mean_j,FC,nlg_p,FDR,file_in);
                % disp(gene_index)
                if isplot==2
                    plotVarianceLink(tLocal)
...
if isplot==1
    ...
    my_heatmap(log2(M_scaled(G_sort,S_sort)+1),[1:length(S_sort)],[]);
